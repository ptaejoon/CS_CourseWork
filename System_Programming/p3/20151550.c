...
int checkFirstToken(char *firstTok)
{
    ...
    if (strcmp("progaddr",firstTok) == 0)return progaddr;
    if (strcmp("loader",firstTok) == 0)return loader;
    if (strcmp("run",firstTok) == 0)return run;
    if (strcmp("bp",firstTok) == 0)return bp;
    return fail;
}
/*
...
{
    if (firstTok == dump || firstTok == bp)return firstTok;
    else if(firstTok == help || firstTok == dir || firstTok == quit || firstTok == history || firstTok ==reset||firstTok == opcodelist || firstTok == symbol||firstTok == run)
    {
...
int executeCommand(int command, char* Arg2,char* Arg3,char* Arg4,char* Arg5,historyLink* Hist,opCode* Op[])
{
    ...
    {
	if(validProgaddrArg(Arg2,Arg3) == fail)return fail;
	return executeProgaddr(Arg2);
    }
    else if(command == loader)
    {
	int loaderArgNum = validLoaderArg(Arg2,Arg3,Arg4,Arg5);
	if(loaderArgNum == fail)return fail;
	return executeLoader(Arg2,Arg3,Arg4,loaderArgNum);
    }
    else if(command == bp)
    {
	int bpArgNum = validBpArg(Arg2,Arg3);
	if(bpArgNum == fail)return fail;
	return executeBp(Arg2,bpArgNum);
    }
    else if(command == run)return executeRun();
    else return fail;
}
/*
...
void dumpNoArg()
{
	...
	{
	    printf("%02x ",sicMemory[count][byte] & 0xFF);
	}
	printf(";");
	for(byte=0; byte < 16; byte++)
	{
	    if((unsigned char)(sicMemory[count][byte]&0xff) < 0x20 ||(unsigned char)(sicMemory[count][byte]&0xff) > 0x7E)printf(".");
	    else printf("%c",sicMemory[count][byte]&0xff);
	}
	printf("\n");
    }
    dumpAddress = count;
...
void dumpWithArgs(int h1, int h2)
{
	...
	for(byte=0; byte < 16; byte++)
	{
		...
		else printf("%02x ",sicMemory[count][byte] & 0xFF);
	    }
	    else printf("%02x ",sicMemory[count][byte] & 0xFF);
	}
	printf("; ");
	for(byte=0; byte < 16; byte++)
	{
		...
		if(byte > h2%16)printf(".");
		else if((unsigned char)(sicMemory[count][byte]&0xff) < 0x20 || (unsigned char)(sicMemory[count][byte]&0xff) > 0x7E)printf(".");
		else printf("%c",sicMemory[count][byte]&0xff);
	    }
	    else if((unsigned char)(sicMemory[count][byte]&0xff) < 0x20 ||(unsigned char)( sicMemory[count][byte]&0xff) >0x7E)printf(".");
	    else printf("%c",sicMemory[count][byte]&0xff);
