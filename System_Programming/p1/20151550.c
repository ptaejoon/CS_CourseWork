...
int checkSecondToken(char *secTok,int firstTok)
{
    ...
    else if(firstTok == help || firstTok == dir || firstTok == quit || firstTok == history || firstTok ==reset||firstTok == opcodelist)
    {
...
void buildOpTable(opCode* head[])
{
	...
	if(strlen(format) == 1)allocator->format[0] = atoi(format);
	    ...
	    allocator->format[0] = format[0]-'0';
	    allocator->format[1] = format[1]-'0';
	}
    }
}
/*
