...
//strings needed to connect Database 
int main(void) {
	...
	//Queries to execute type commands.
	int state = 0;
	int write = 0;
	char create_q[1001];
	char truck[20];
	...
	else
	{
		...
		while (1) {
			...
			if (user_input[0] != '0' && select_q == 0)select_q = 6; //send to default when number is not in between 0 to 5
			switch (select_q)
			{
				case 0:
					printf("HAVE A GOOD DAY :) \n");
					state = mysql_query(connection, drop_db);
				...
				case 1://type 1
					...
					while (1)
					{
						...
						if (truck_correct == 0)
						{
							...
							//search truck that had accident before and compare with user designated truck
							//if delivery_record has arrival_time=NULL and name!="recipient", it had accident
							//if the truck destroyed, set truck_correct 1
						...
						switch (select_q)
						{
							...
							case 1: // subtype 1-1
								...
								if (state == 0)
								{
									...
									//print id too in case of same name but different person
									mysql_free_result(sql_result);
								}
								else
								{
							...
							case 3: // subtype 1-3
								...
								if (state == 0)
								{
									...
									mysql_free_result(sql_result);
									//query find last record by check delivery record of the truck.
									//if arrival time is earlier than accident's start time and has earliest record, it is the last record 
								}
								else
								{
									printf("Must be Error on Query.\n");
				...
				case 2: //subtype 2
					while (1)
					{
						...
						if (state == 0)
						{
							...
							mysql_free_result(sql_result);
							//print customer id too in case of same name but different person.
						}
						else
						{
							printf("Must be Error on Query.\n");
				...
				case 3: //type 3
					...
					break;
				case 4: //type4
					//type4 just prints package's id that had been late on delivery.
					//don't need to care input
					printf("---- TYPE IV ----\n\n");
					...
					break;
				case 5: // type 5
					//best example : 3(customer id), 2020-05(which month?)
					while (1)
					{
						...
						if (state == 0)
						{
							...
							//while fetching information, add charges
							mysql_free_result(sql_result);
							if (write == 0)
							{
